...
class Transaction(ComparableMixin, Base):
    ...
    _amt = Column(sqlalchemy.types.Float)        
    _exempt = Column(sqlalchemy.types.Boolean)  
    _date = Column(sqlalchemy.types.Date)   # sqlalchemy.types.Datetime   
    ...
    def __init__(self, amt, acct_num, date=None, exempt=False):
        ...
        self._amt = amt
        self._date = date 
        if not date: # self._date
            self._date = datetime.now().date()
        # if not date: # self._date
        #     self._date = datetime.now().date()
        # else:
        #     self._date = datetime.strptime((str) (date), "%Y-%m-%d").date() # input("Date? (YYYY-MM-DD)\n>")
        self._exempt = exempt
