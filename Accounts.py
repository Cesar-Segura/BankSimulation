...
class Account(Base):
    ...
    _bank = Column(Integer, ForeignKey("bank._id"))  
    _transactions = relationship("Transaction") 
    _type = Column(String)  
    _interest_rate = Column(sqlalchemy.types.Float)
...
class SavingsAccount(Account, Base):
    ...
    __mapper_args__ = {'polymorphic_identity':'SavingsAccount'}
...
class CheckingAccount(Account, Base):
    ...
    __mapper_args__ = {'polymorphic_identity':'CheckingAccount'} 
